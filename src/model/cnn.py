...
class CNN(nn.Module):
    def __init__(self, num_features=3, hidden_dim=128, num_users=40, num_channels=20, device="cpu"):
        super().__init__()
        self.N = num_users
        self.K = num_channels
    ...
    def forward(self, prev_state, state):
        ...
        mask = self.get_mask(prev_state, state)
        out = self.net(state)
        output = out.masked_fill(mask, float("-inf"))

        return output

    def get_mask(self, prev_state, state):
